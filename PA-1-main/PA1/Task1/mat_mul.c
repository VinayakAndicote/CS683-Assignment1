...
//----------------------------------------------------- Write your code here ----------------------------------------------------------------
	for (int i = 0; i < size; i++) {
        for (int k = 0; k < size; k++) {
            __m128d a_val = _mm_set1_pd(A[i * size + k]); // broadcast
            int j;
            for (j = 0; j <= size - 2; j += 2) {
                __m128d b_vec = _mm_loadu_pd(&B[k * size + j]);
                __m128d c_vec = _mm_loadu_pd(&C[i * size + j]);
                __m128d res   = _mm_add_pd(c_vec, _mm_mul_pd(a_val, b_vec));
                _mm_storeu_pd(&C[i * size + j], res);
            }
            for (; j < size; j++) {
                C[i * size + j] += A[i * size + k] * B[k * size + j];
            }
     ...
     for (int ii = 0; ii < size; ii += tile_size) {
        for (int kk = 0; kk < size; kk += tile_size) {
            for (int jj = 0; jj < size; jj += tile_size) {
                for (int i = ii; i < ii + tile_size && i < size; i++) {
                    for (int k = kk; k < kk + tile_size && k < size; k++) {
                        __m128d a_val = _mm_set1_pd(A[i * size + k]); // broadcast A[i][k]
                        int j;
                        // process 2 doubles at a time
                        for (j = jj; j <= jj + tile_size - 2 && j <= size - 2; j += 2) {
                            __m128d b_vec = _mm_loadu_pd(&B[k * size + j]);
                            __m128d c_vec = _mm_loadu_pd(&C[i * size + j]);
                            __m128d res   = _mm_add_pd(c_vec, _mm_mul_pd(a_val, b_vec));
                            _mm_storeu_pd(&C[i * size + j], res);
