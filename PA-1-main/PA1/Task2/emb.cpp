...
long long run_with_prefetching(const vector<float>& embedding_table, const vector<int>& input, const vector<int>& offsets) {
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
        ...
        for (int j = start_idx; j < end_idx; ++j) {
            ...
            // Software prefetch: prefetch embedding for the "next" index
            if (j + 1 < end_idx) {
                _mm_prefetch((const char*)&embedding_table[input[j+1] * embedding_dim], _MM_HINT_T0);
            }
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
        ...
        for (int j = start_idx; j < end_idx; ++j) {
            ...
            for (; d <= embedding_dim - 4; d += 4) {
                __m128 a = _mm_loadu_ps(&data_ptr[d]);
                __m128 b = _mm_loadu_ps(&bag_embedding[d]);
                b = _mm_add_ps(a, b);
                _mm_storeu_ps(&bag_embedding[d], b);
            }
            // leftovers
...
long long run_with_prefetching_simd(const vector<float>& embedding_table, const vector<int>& input, const vector<int>& offsets) {
    ...
    for (size_t i = 0; i < offsets.size(); ++i) {
        ...
        for (int j = start_idx; j < end_idx; ++j) {
            ...
            // Software prefetch for next embedding
            if (j + 1 < end_idx) {
                _mm_prefetch((const char*)&embedding_table[input[j+1] * embedding_dim], _MM_HINT_T0);
            }
